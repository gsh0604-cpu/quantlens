...
export default function Home() {
  ...
  const rangePos  = data?.high52 && data?.low52
    ? Math.min(98, Math.max(2, ((data.price - data.low52) / (data.high52 - data.low52)) * 100))
    : 50;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#060c16", color: "#e2e8f0", fontFamily: "'IBM Plex Mono','Courier New',monospace" }}>
      ...
      <style>{`*{box-sizing:border-box;margin:0;padding:0}input:focus{outline:none}@keyframes spin{to{transform:rotate(360deg)}}@keyframes fadeIn{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}.fade-in{animation:fadeIn 0.3s ease forwards}`}</style>
      ...
      <div style={{ borderBottom: "1px solid #1a2a3a", padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8, position: "sticky", top: 0, background: "#060c16", zIndex: 10 }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
          ...
          />
          <button onClick={() => fetchStock(input)}
            style={{ background: "#00ff8818", border: "1px solid #00ff8840", color: "#00ff88", padding: "7px 14px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontFamily: "monospace", fontWeight: 700, letterSpacing: "0.1em" }}>
            GO
          </button>
          <Link href="/portfolio"
            style={{ background: "#a78bfa18", border: "1px solid #a78bfa40", color: "#a78bfa", padding: "7px 12px", borderRadius: 6, fontSize: 10, fontFamily: "monospace", fontWeight: 700, letterSpacing: "0.08em", textDecoration: "none", whiteSpace: "nowrap" }}>
            ETF OPTIMIZER →
          </Link>
          <Link href="/positions"
      ...
      {/* ── Watchlist ── */}
      <div style={{ padding: "8px 16px", display: "flex", gap: 6, borderBottom: "1px solid #1a2a3a", flexWrap: "wrap" }}>
        {WATCHLIST.map(w => (
          <button key={w} onClick={() => { setInput(w); fetchStock(w); }}
            style={{ background: ticker === w ? "#00ff8812" : "transparent", border: `1px solid ${ticker === w ? "#00ff8840" : "#1a2a3a"}`, color: ticker === w ? "#00ff88" : "#3d5a70", padding: "5px 12px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontFamily: "monospace", letterSpacing: "0.05em" }}>
            {w}
          </button>
        ))}
      </div>
      ...
      {data && mods && !loading && (
        <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: 14, maxWidth: 800, margin: "0 auto" }} className="fade-in">
          ...
          {/* Fundamentals */}
          {data.info && Object.values(data.info).some(Boolean) && (
            <div>
