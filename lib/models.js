...
export function computeFactors(info) {
  const pe   = info?.trailingPE    || info?.forwardPE    || 0;
  const pb   = info?.priceToBook   || 0;
  const roe  = (info?.returnOnEquity  || 0) * 100;
  const roa  = (info?.returnOnAssets  || 0) * 100;
  const rev  = (info?.revenueGrowth   || 0) * 100;
  const earn = (info?.earningsGrowth  || 0) * 100;
  const mgn  = (info?.profitMargins   || 0) * 100;
  const de   = info?.debtToEquity  || 0;

  // Value: lower P/E and P/B = better value
  const peScore = pe  > 0 ? Math.max(0, Math.min(100, 100 - pe  * 1.5)) : null;
  const pbScore = pb  > 0 ? Math.max(0, Math.min(100, 100 - pb  * 8))   : null;
  const valueScore = peScore !== null || pbScore !== null
    ? ((peScore ?? 50) * 0.6 + (pbScore ?? 50) * 0.4)
    : 50;

  // Quality: ROE, ROA, margins, low debt
  const roeScore = roe > 0 ? Math.min(100, roe * 2.5)  : 0;
  const roaScore = roa > 0 ? Math.min(100, roa * 5)    : 0;
  const mgnScore = mgn > 0 ? Math.min(100, mgn * 2)    : 0;
  const deScore  = de  > 0 ? Math.max(0, 100 - de * 2) : 70;
  const qualityScore = roe > 0 || mgn > 0
    ? (roeScore * 0.35 + roaScore * 0.2 + mgnScore * 0.25 + deScore * 0.2)
    : 0;

  // Growth: revenue + earnings growth
  const revScore  = Math.min(100, Math.max(0, rev  * 2 + 50));
  const earnScore = Math.min(100, Math.max(0, earn * 1.5 + 50));
  const growthScore = rev !== 0 || earn !== 0
    ? (revScore * 0.6 + earnScore * 0.4)
    : 50;

  return {
    value:   { score: valueScore.toFixed(0),   raw: `P/E: ${pe?.toFixed(1) || "N/A"}, P/B: ${pb?.toFixed(1) || "N/A"}` },
    quality: { score: qualityScore.toFixed(0), raw: `ROE: ${roe?.toFixed(1) || "N/A"}%, Mgn: ${mgn?.toFixed(1) || "N/A"}%` },
    growth:  { score: growthScore.toFixed(0),  raw: `Rev: ${rev > 0 ? "+" : ""}${rev?.toFixed(1) || "N/A"}%` },
  };
}
